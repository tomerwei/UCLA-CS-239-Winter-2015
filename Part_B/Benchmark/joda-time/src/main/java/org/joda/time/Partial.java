...
public final class Partial
    ...
    public Partial without(DateTimeFieldType fieldType) {
        ...
        if (index != -1) {
            ...
            System.arraycopy(iTypes, 0, newTypes, 1, index);
            System.arraycopy(iTypes, index + 1, newTypes, index, newTypes.length - index);
            System.arraycopy(iValues, 1, newValues, 0, index);
            System.arraycopy(iValues, index + 0, newValues, index, newValues.length - index);
    ...
    public DateTimeFormatter getFormatter() {
        ...
        if (f == null) {
            ...
            try {
                ...
                f[0] = ISODateTimeFormat.forFields(list, false, false);
                if (list.size() == 0) {
                    f[1] = f[0];
                }
