/*
 * TestPatternMatching.java: JUnit test for regular expression tokenizing
 *
...
public class TestPatternMatching extends TestCase {
  ...
  public static void main(String[] args) {
    String[]   tests = { TestPatternMatching.class.getName() };
  ...
  public static Test suite() {
    TestSuite   suite = new TestSuite(TestPatternMatching.class.getName());
    
    suite.addTest(new TestPatternMatching("compareJTopasAgainstPattern"));
    suite.addTest(new TestPatternMatching("testNumberPattern"));
    suite.addTest(new TestPatternMatching("testPatternExt"));
    suite.addTest(new TestPatternMatching("testPatternTokenizer"));
    suite.addTest(new TestPatternMatching("testFreePattern"));
    return suite;
  }
   ...
   */  
  public TestPatternMatching(String test) {
    super(test);
  }
  ...
  public void testPatternTokenizer() throws Throwable {
    ...
    props.addPattern("[a-z][a-z0-9]*", identifier, TokenizerProperties.F_NO_CASE);
    props.addPattern("//.*$", lineComment);
  ...
  public void testFreePattern() throws Throwable {
    ...
    props.addPattern("[+\\-]?[ \t]*[0-9]+\\.?[0-9]*", number,      TokenizerProperties.F_FREE_PATTERN);
    props.addPattern("[a-z][a-z0-9]*",                identifier,  TokenizerProperties.F_NO_CASE);
    props.addPattern("//.*$",                         lineComment, TokenizerProperties.F_FREE_PATTERN);
    props.addPattern("\\(.*\\)",                      paraList,    TokenizerProperties.F_FREE_PATTERN);
    props.addSpecialSequence("{", separator);
    props.addSpecialSequence("}", separator);
    props.addSpecialSequence("(", separator);
    props.addSpecialSequence(")", separator);
  ...
  public void compareJTopasAgainstPattern() throws Throwable {
    ...
    TokenizerProperties jtopasProps       = new StandardTokenizerProperties(TokenizerProperties.F_RETURN_WHITESPACES | TokenizerProperties.F_TOKEN_POS_ONLY);
    TokenizerProperties patternProps      = new StandardTokenizerProperties(TokenizerProperties.F_RETURN_WHITESPACES | TokenizerProperties.F_TOKEN_POS_ONLY);
    StandardTokenizer   jtopasTokenizer   = new StandardTokenizer(jtopasProps);
    StandardTokenizer   patternTokenizer  = new StandardTokenizer(patternProps);

    // patternProps.addPattern("/\\*\\*.*\\*/",            docComment,   TokenizerProperties.F_FREE_PATTERN);
    patternProps.addPattern("/\\*.*?\\*/",                blockComment, TokenizerProperties.F_FREE_PATTERN);
    patternProps.addPattern("//.*?$",                     lineComment,  TokenizerProperties.F_FREE_PATTERN);
    patternProps.addPattern("\"[^$\"]*?\"",               string,       TokenizerProperties.F_FREE_PATTERN);
    // patternProps.addPattern("\"[[^$\"]|[\\\\&&\"]]*?\"", string,       TokenizerProperties.F_FREE_PATTERN);
    patternProps.addPattern("'.'",                        character,    TokenizerProperties.F_FREE_PATTERN);
    // patternProps.addPattern("'[[^']|[\\\\&&']]+?'",       character,    TokenizerProperties.F_FREE_PATTERN);
    ...
    patternTokenizer.setSource(reader);
    jtopasTokenizer.setSource(reader);
    
    tokenize(patternTokenizer, reader);
    tokenize(jtopasTokenizer, reader);
  }
   ...
   */
  private void tokenize(StandardTokenizer tokenizer, Reader reader) throws Throwable {
