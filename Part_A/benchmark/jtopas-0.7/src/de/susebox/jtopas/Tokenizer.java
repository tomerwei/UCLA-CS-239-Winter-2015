...
/**<p>
 * The interface <code>Tokenizer</code> contains setup methods, parse operations 
 ...
 * etc. These units are the tokens that are reflected in the {@link Token} class
 * of the <code>de.susebox.jtopas</code> package.
 *</p><p> 
 * A <code>Tokenizer</code> is configured using a {@link TokenizerProperties} 
 ...
 * (see {@link StandardTokenizerProperties} and {@link StandardTokenizer} for the
 ...
 * {@link java.io.InputStreamReader}, database queries, string arrays etc. The
 * interface {@link TokenizerSource} serves as an abstraction of such widely 
 * varying data sources.
 *</p><p>
 * The <code>Tokenizer</code> interface partly replaces the older 
 * {@link de.susebox.java.util.Tokenizer} interface which is deprecated.
 *</p>
 *
 * @see     Token
...
public interface Tokenizer {
   ...
   * associated with this <code>Tokenizer</code>.
   *
   ...
   * the parse process. A common place is a constructor of a <code>Tokenizer</code>
   * implementation. If the tokenizer characteristics change during the parse
   * process they take effect with the next call of {@link #nextToken} or 
   * {@link #nextImage}. Usually, a <code>Tokenizer</code> implementation will
   * also implement the {@link TokenizerPropertyListener} interface to be notified
   * about property changes. 
   *<br>
   * Generally, the <code>Tokenizer</code> implementation should also implement
   * the {@link de.susebox.jtopas.spi.DataProvider} interface or provide an inner
   * class that implements the <code>DataProvider</code> interface, while the
   * {@link TokenizerProperties} implementation should in turn implement the
   * interfaces 
   *<ul><li>
   *    {@link de.susebox.jtopas.spi.WhitespaceHandler}, 
   *</li><li>
   *    {@link de.susebox.jtopas.spi.SeparatorHandler}, 
   *</li><li>
   *    {@link de.susebox.jtopas.spi.SequenceHandler}, 
   *</li><li>
   *    {@link de.susebox.jtopas.spi.KeywordHandler} and 
   *</li><li>
   *    {@link de.susebox.jtopas.spi.PatternHandler}
   *</li></ul>
   * These handler interfaces are collected in the {@link de.susebox.jtopas.spi.DataMapper}
   * interface.
   *<br>
   * Although the implementation of the mentioned interfaces is recommended, it 
   * is not a mandatory way. Except for {@link de.susebox.jtopas.spi.PatternHandler}
   * that must be implemented by the {@link TokenizerProperties} implementation,
   * since it is not possible for a <code>Tokenizer</code> to interpret a regular
   * expression pattern only with the information provided through the 
   * <code>TokenizerProperties</code> interface. 
   *<br>
   * If a <code>Tokenizer</code> implementation chooses to use a exclusively tailored 
   * {@link TokenizerProperties} implementation, it should throw an 
   * {@link java.lang.IllegalArgumentException} if it is not provided with an 
   * instance of that {@link TokenizerProperties} implementation.
   *<br>
   ...
   * <code>null</code> if {@link #setTokenizerProperties} has not been called so 
   * far.
   *
   ...
   * to a certain <code>Tokenizer</code> instance, can be set using this method.
   *
   ...
   *<br>
   * Usually, the {@link TokenizerProperties} used by a <code>Tokenizer</code>
   * implement the {@link de.susebox.jtopas.spi.WhitespaceHandler} interface. If
   * so, the <code>Tokenizer</code> object sets the <code>TokenizerProperties</code> 
   * instance as its <code>WhitespaceHandler</code>. A different handler or a 
   * handler specific to a certain <code>Tokenizer</code> instance, can be set 
   * using this method.
   *
  ...
  /**
   * Retrieving the current {@link de.susebox.jtopas.spi.WhitespaceHandler}. The 
   * method may return <code>null</code> if there whitespaces are not recognized.
   *
   ...
   * previously installed <code>SeparatorHandler</code>. If <code>null</code> is 
   * passed, the tokenizer doesn't recognize separators.
   *<br>
   * Usually, the {@link TokenizerProperties} used by a <code>Tokenizer</code>
   * implement the {@link de.susebox.jtopas.spi.SeparatorHandler} interface. If
   * so, the <code>Tokenizer</code> object sets the <code>TokenizerProperties</code> 
   * instance as its <code>SeparatorHandler</code>. A different handler or a 
   * handler specific to a certain <code>Tokenizer</code> instance, can be set 
   * using this method.
   *
  ...
  /**
   * Retrieving the current {@link de.susebox.jtopas.spi.SeparatorHandler}. The 
   * method may return <code>null</code> if there isn't any handler installed.
   *
   ...
   *<br>
   * Usually, the {@link TokenizerProperties} used by a <code>Tokenizer</code>
   * implement the {@link de.susebox.jtopas.spi.SequenceHandler} interface. If
   * so, the <code>Tokenizer</code> object sets the <code>TokenizerProperties</code> 
   * instance as its <code>SeparatorHandler</code>. A different handler or a 
   * handler specific to a certain <code>Tokenizer</code> instance, can be set 
   * using this method.
   ...
   *<br>
   * Usually, the {@link TokenizerProperties} used by a <code>Tokenizer</code>
   * implement the {@link de.susebox.jtopas.spi.PatternHandler} interface. If
   * so, the <code>Tokenizer</code> object sets the <code>TokenizerProperties</code> 
   * instance as its <code>PatternHandler</code>. A different handler or a 
   * handler specific to a certain <code>Tokenizer</code> instance, can be set 
   * using this method.
   *
  ...
  /**
   * Check if there are more tokens available. This method will return
   * <code>true</code> until and enf-of-file condition is encountered during a 
   * call to {@link #nextToken} or {@link #nextImage}.
   *<br>
  ...
  /**
   * Retrieving the next {@link Token}. The method works in this order:
   *<ol><li>
   ...
   *   a regular expression that is used by {@link java.util.regex.Pattern}. But
   ...
   * or {@link #nextImage}. 
   *<br>
   * Since version 0.6.1 of JTopas, this method throws a {@link TokenizerException}
   * rather than returning <code>null</code> if neither {@link #nextToken} nor 
   * {@link #nextImage} have been called before or {@link #setReadPositionRelative}
   * or {@link #setReadPositionAbsolute} habe been called after the last call to
   * <code>nextToken</code> or <code>nextImage</code>.
   *
  ...
  /**
   * If the flag {@link TokenizerProperties#F_COUNT_LINES} is set, this method 
   * will return the line number starting with 0 in the input stream. The 
   * implementation of the <code>Tokenizer</code> interface can decide which 
   ...
   * line number.
   *
   * @return  the current line number starting with 0 or -1 if no line numbers 
   *          are supplied ({@link TokenizerProperties#F_COUNT_LINES} is not set).
   * @see #getColumnNumber
   */  
   ...
   * column number.
   *
   * @return  the current column position or -1 if the flag if no column numbers
   *          are supplied {@link TokenizerProperties#F_COUNT_LINES} is not set).
   *          is not set
   * @see     #getLineNumber
   */  
   ...
   * {@link #nextToken}, if that token is no whitespace or if whitespaces are
   * returned ({@link  TokenizerProperties#F_RETURN_WHITESPACES}).
   *<br>
   ...
   * {@link #currentlyAvailable} would return.
   *
   ...
   *<br>
   * The current read position is the end position of the current token. That means
   * that the following assertion can be made:
   *<pre>
   ...
   *</pre>
   *<br>
   * Since JTopas version 0.6.1, the operation clears the current token. Therefore,
   * {@link #currentImage} and {@link #currentToken} will throw a {@link TokenizerException}
   * if called after a <code>setReadPositionAbsolute</code> without a subsequent
   * call to {@link #nextToken} of {@link #nextImage}.
   *
   ...
   * a <code>Tokenizer</code> instances are frequently created for small tasks.
