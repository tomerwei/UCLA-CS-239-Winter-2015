...
java -mx48m -cp ${CLASSPATH}:${JAVA_HOME}/lib/classes.zip -Dant.home=bootstrap -Dbuild.tests=build/testcases -Dtests-classpath.value=${CLASSPATH} listclass -nocontents -dependencies -recurse ${DLIST} -exclude java. javax. sun.
) |
sort -u
